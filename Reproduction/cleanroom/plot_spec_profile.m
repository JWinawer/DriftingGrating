...
% error bar would be roughly an order of magnitude too small.
    ...
    % This figure shows BOTH experiments, so one "n" is only honest while they share an
    % observer list. dg can be 13 where da is 7; say so rather than printing one number.
    nDG = numel(S.dg.subjects);  nDA = numel(S.da.subjects);
    if nDG == nDA, nTxt = sprintf('n = %d', nDG);
    else,          nTxt = sprintf('n = %d (dg), %d (da)', nDG, nDA); end
    title(tl, {sprintf(['The three contrasts measurable at one polar angle  |  %s, ' ...
               '%g-%g%s  |  %s'], S.area, S.eccRange(1), S.eccRange(2), char(176), ...
               nTxt), ...
