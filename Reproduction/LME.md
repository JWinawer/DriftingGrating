# The linear mixed-effects model (Fig 7) — what it adds, and why it comes out

**Resolved, and adopted.** The LME estimates nothing the beta-subtraction route does not, and its
standard error is wrong in a known direction. **Figure 7 is being removed** (JW, 2026-08-19).
Report instead the within-observer error summary in the data-quality section
([`local_qc/REPORT.md`](local_qc/REPORT.md) §2.7,
[`local_qc/GLM_SUMMARY_SECTION.md`](local_qc/GLM_SUMMARY_SECTION.md)), and point to this repository
for the precision-weighted check.

§§1–4 are why. §5 is the precision-weighted analysis that stands in its place, and is the part of
this document with ongoing use. §7 flags an interval-method disagreement that still needs a
decision.
...
a single observer makes the routes diverge by ~1.2e-2. Not a concern at the manuscript inclusion
criteria; it would become one if a stricter filter emptied a wedge.

*The values in this table are on the wedge-**median** route, because the point is the exact
equality of the two routes on identical input, not the estimates themselves. The manuscript
aggregate is the **mean** (`../AGENTS.md` standing fact 4); the equality holds under either. The
two routes as the manuscript has them once differed on this — `lme1_fit.m` read `meanBOLDpa` while
`plot_NeuralAsymmetries.m` passed `medianBOLDpa` — which was the whole of their apparent
disagreement, and was fixed 2026-08-19.*
...
*wᵢ* = 1/(τ² + σᵢ²). Generated by `cleanroom/precision_weighted_table.m` and written to
`supplement/precision_weighted.csv`. Intervals are 95% on 7 df.

*On the manuscript route — across-vertex **mean** with observer pRF-gain rescaling (regenerated
2026-08-19). The median-route tables this replaced are recoverable from git history; the conclusion
was the same under both.*
...
**Precision weighting moves no context effect and no Cartesian-frame asymmetry**, which is what
the manuscript's claims rest on. The one cell whose *status* it changes is **da rad−tang**,
0.150 [0.007, 0.292] → 0.176 [0.065, 0.286]. That cell is marginal by every route (§7) and should
not be reported as though weighting settled it. It is also the cell with the most heterogeneous
reliability (*w* max/min 9.89×), and the reweighting acts by down-weighting sub-0426 (σ = 0.251)
and sub-0395 (σ = 0.158, 6 runs rather than 8) — sub-0395 being the observer the leave-one-out
analysis already singles out. So precision weighting is a principled version of that probe, moving
in the same direction; and it still does **not** make the rad−tang *context* effect significant
(p = 0.39 vs 0.56), which strengthens the "uninformative, not null" reading of the polar-frame
result rather than undermining it.

Note that the gain rescaling and the precision weighting interact: *w* max/min for da rad−tang is
9.89× here against 2.95× on the un-rescaled median route (see the "double weighting" note in
[`local_qc/GLM_SUMMARY_SECTION.md`](local_qc/GLM_SUMMARY_SECTION.md) Result 4).
...
[`local_qc/REPORT.md`](local_qc/REPORT.md) §2.7) bounds what any of this can achieve.
...
- the within-observer error summary, in the data-quality section — it shows both that the GLM data
  are good and that precision weighting is not needed
  ([`local_qc/REPORT.md`](local_qc/REPORT.md) §2.7);
...
Adopted 2026-08-19. One consequence is not yet settled: **Figure 8 plots this same LME's estimate
in red** against the observer data in black, so whether Fig 8 keeps the model overlay is an open
question (`../AGENTS.md` §7).
...
`cleanroom/fit_lme_fig7.m` (the model as the manuscript specifies it), `cleanroom/lme_codes.m` (the asymmetry codes),
