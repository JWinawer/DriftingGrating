...
| `GLM_QUALITY.md` | the **first** GLM fit-quality audit, on the 4–8°-filtered extraction | **superseded.** `../local_qc/REPORT.md` is the second pass on the unfiltered re-extraction this document's §6a called for, and its §2.1 table is strictly richer. Its own banner already marked its interpretations superseded. Unique to it: the `fracAbove` scale argument (§3) and the median-understates-sessions point (§4a), both about the first extraction. Its §7 measurement-reliability section was **moved** to `../local_qc/REPORT.md` §2.7. |
