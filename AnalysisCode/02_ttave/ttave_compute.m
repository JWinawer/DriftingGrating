function ttave_compute(matrices, datafiles_origin, modelfiles_origin, asymmetryName, surfaceROI, projectSettings, varargin)

for fititer = 1:2 % one for data, one for model

    if fititer==1
        datafiles = datafiles_origin;
        savename = 'data';
    elseif fititer==2
        datafiles = modelfiles_origin;
        savename = 'model';
    end
    ...
    for mi=1:length(matrices)
        ...
        for pa=1:length(polarAngleConditions) 
            ...
            elseif fititer==2
                df_roi = nanmedian(df.*surfaceSelection); % model fit already in PSC
            end
    ...
    ttaveOutput.(savename) = {base, blank, advMotion, disadvMotion, advStatic, disadvStatic};
    %ttaveTime = {eventTRs_prior, eventTRs_after};
    
    projectSettings.secondary.(savename) = secondary;
end
