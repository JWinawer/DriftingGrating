...
| 7 | **Extrastriate** | Does the V1 route work in maps with empty polar-angle ROIs? | **No, and it is fixed.** Empty cells are not random in polar angle, so the ROI average changes estimand. The harmonic route, applied identically to every map including V1, does not bin and so has no cells to lose; V1 is unchanged to 1.5e-14. | [`Reproduction/EXTRASTRIATE.md`](Reproduction/EXTRASTRIATE.md) |
