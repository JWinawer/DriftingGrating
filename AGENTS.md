...
| 7 | **Extrastriate** | Does the V1 route work in maps with empty polar-angle ROIs? | **No, and it is replaced.** One specification now applies unchanged to every map: per-map gain, harmonic model with continuous polar angle, equal coverage at 45 deg, equal weighting across observers. Adopting it moves V1 by at most 0.040 and changes no significance. | [`Reproduction/EXTRASTRIATE.md`](Reproduction/EXTRASTRIATE.md) |
